...
from completion.preview_viewer import write_interactive_preview
...
def load_puml() -> str:
    ...
    puml_path = resources.files("completion").joinpath(PUML_RELATIVE_PATH)
...
def build_kroki_preview_url(
    ...
    *,
    ...
    """Construit une URL Kroki GET (SVG statique, sans zoom)."""
...
def _default_html_path() -> Path:
    return Path(tempfile.gettempdir()) / "inference-architecture-preview.html"
...
def run_preview(
    *,
    ...
    """Lance la preview architecture (viewer HTML zoom/pan + SVG via Kroki)."""
    try:
        puml = load_puml()
        preview_url = build_kroki_preview_url(puml, kroki_base_url=kroki_base_url)
    except FileNotFoundError:
        print(
            f"Erreur : {PUML_RELATIVE_PATH} introuvable dans le package completion.",
            file=sys.stderr,
        )
