...
def colorize_help(text: str) -> str:
    if not supports_color():
        return text
    return "\n".join(_colorize_help_line(line) for line in text.splitlines())
...
def _colorize_help_line(line: str) -> str:
    ...
    styled = line
    styled = re.sub(r"\b(QUERY_SENTINEL_\w+)\b", rf"{BOLD}{MAGENTA}\1{RESET}", styled)
    styled = re.sub(r"\b(X-Django-[A-Za-z-]+)\b", rf"{YELLOW}\1{RESET}", styled)
    styled = re.sub(r"\b(query_sentinel\.[\w.]+)\b", rf"{BLUE}\1{RESET}", styled)
