...
from payload_sentinel.exceptions import ConfigurationError
...
def configure_from_file(path: str | Path) -> None:
    ...
    if not isinstance(raw, dict):
        raise ConfigurationError("Le fichier JSON doit être un objet à la racine.")
...
def get_setting(name: str, default: Any = _MISSING) -> Any:
    ...
    raise ConfigurationError(
        "Aucune configuration active. Utilisez --settings ou --config."
    )
...
def bootstrap_runtime(
    *,
    ...
    raise SystemExit(
        ...
        "  Standalone : --config ./payload.json"
    )
...
def _bootstrap_django(settings_module: str | None) -> None:
    ...
    try:
        ...
        raise SystemExit(f"Impossible d'importer '{module}' ({exc}).") from exc
